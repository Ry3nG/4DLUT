...
NUM_IMAGES = 50  # Number of image pairs to download
...
def main():
    ...
    with open(CATEGORIES_FILE, 'r') as f:
        image_names = [line.split(',')[0] for line in f][:NUM_IMAGES]
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        successful_images = []
        with tqdm(total=len(image_names), desc="Downloading images") as pbar:
    ...
    split_index = int(len(successful_images) * 0.8)  # 80% for training, 20% for testing
    ...
    print(f"Total images: {len(successful_images)}")
